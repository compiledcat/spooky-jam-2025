...
public class SpaceshipController : MonoBehaviour
{
    ...
    [SerializeField] private float maxCamLookaheadDistance = 0.8f; //Multiplied by the camera's half-height
    ...
    private void LateUpdate()
    {
        ...
        //The maximum distance the camera can move ahead of the spaceship is the maxCamLookaheadDistance * half the camera's orthographic height
        float t = Mathf.InverseLerp(0.0f, maxLinearVelocity, rb.linearVelocity.magnitude);
        float camLookaheadDistance = Mathf.Lerp(0.0f, maxCamLookaheadDistance, t);
        Vector2 newCamPos = Vector2.Lerp(cam.transform.position, transform.position + rb.linearVelocity.normalized * camLookaheadDistance * cam.orthographicSize, camLinearLerpSpeed * Time.deltaTime);
